...
import { AlignLeft, Columns2, Copy, FileText, GitCommitHorizontal, Loader2, Maximize2, Minimize2, X } from "lucide-react";
...
export function DiffSheet({ open, onOpenChange, repoPath, filePath, staged }: DiffSheetProps) {
  ...
  // Dialog sizing: normal = near-full width, expanded = 98%
  const dialogSize = expanded
    ? "max-w-[98vw] w-[98vw] h-[98vh]"
    : "w-[50vw] max-w-[95vw] h-[65vh]";

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={false} className={`${dialogSize} flex flex-col p-0 gap-0 transition-all duration-200`}>
        <DialogHeader className="px-4 py-2.5 border-b border-border shrink-0">
          <DialogTitle className="flex items-center gap-2 text-sm font-mono min-w-0">
            ...
            <div className="flex items-center gap-0.5 shrink-0 ml-2">
              <Tooltip>
                ...
                <TooltipContent>Unified view (F)</TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                ...
                <TooltipContent>Split view (F)</TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
